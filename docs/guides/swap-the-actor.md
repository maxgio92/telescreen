...
Go, posts the draft verbatim), and you can enroll your own instead. The
normative contract is [thinkpol.md](../contracts/thinkpol.md); this
...
1. Resolve the entry from the approval's `entry <absolute path>` line.
2. Extract the last draft section; no draft, or a discarded marker
   after it, means remove the approval and touch nothing.
3. Post the draft to the entry's URL.
4. On success: append the `--- published <time> <URL>` marker, move the
   entry to `upsub/` (unless it already sits in `upsub/` or `files/`),
   remove the approval.
5. On failure: leave the entry untouched, remove the approval so
...
authenticates, only what it writes to the store.
