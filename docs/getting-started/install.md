...
pane, delete it with `x` `x`. Quit with `q`.
...
[Enroll the agents](enroll.md) to get real records and draft
reactions, or jump straight to [Your first record](first-record.md) on
the demo seed.
